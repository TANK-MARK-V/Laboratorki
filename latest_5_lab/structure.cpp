...
int compare_flight(Flight flight, Flight_filter filter) {
	...
	if (is_good && filter.apply[1]) {
		...
		mystrlwr(strcpy(filter_name, filter.name));
	...
	if (is_good && filter.apply[2]) {
		...
		mystrlwr(strcpy(filter_dest, filter.dest));
...
int edit_structures(Flight** pointer, int len, Flight_filter filter) {
	printf("\nВведите номера столбцов, которые вы хотите изменить, через запятую:\n");
	printf("1 - Номер рейса\n");
	printf("2 - Тип самолёта\n");
	printf("3 - Пункт назначения\n");
	...
	for (int i = 0; i < last_ind && !change_res; i++)
		...
		else if (cols[i] == 5) change_res = input_time(&new_data, 0);
		else if (cols[i] == 6) change_res = input_time(&new_data, 1);
		else if (cols[i] == 7) change_res = input_price(&new_data);

	for (int line = 0; line < len && !change_res; line++) {
		if (!compare_flight((*pointer)[line], filter)) continue;
		for (int i = 0; i < last_ind && !change_res; i++)
			if (cols[i] == 1) (*pointer)[line].fnum = new_data.fnum;
			else if (cols[i] == 2) strcpy((*pointer)[line].name, new_data.name);
			else if (cols[i] == 3) strcpy((*pointer)[line].dest, new_data.dest);
			else if (cols[i] == 4) for (int j = 0; j < 7; j++) (*pointer)[line].days[j] = new_data.days[j];
			else if (cols[i] == 5) (*pointer)[line].dep_time = new_data.dep_time;
			else if (cols[i] == 6) (*pointer)[line].arr_time = new_data.arr_time;
			else if (cols[i] == 7) (*pointer)[line].price = new_data.price;
	}
	return change_res;  // Если ввод полный, то 0 - иначе 1
}